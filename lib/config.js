...
class TelegramConfig {
    ...
    /**
     * Load main configuration (API credentials only)
     * @returns {Promise<Object>} Main configuration object
    ...
    async loadMainConfig() {
        ...
        try {
            ...
            this.mainConfig = yaml.load(mainConfigFile);

            this._validateApiCredentials(this.mainConfig);

            return this.mainConfig;
        } catch (err) {
            throw new Error(`Failed to load main config: ${err.message}`);
        }
    }
    ...
    async loadTelegramConfig() {
        ...
        try {
            ...
            // Load download.yml for chat and sections configuration
            ...
            const downloadConfig = yaml.load(downloadConfigFile);
            ...
            // Validate download-specific configuration components
    ...
    /**
     * Load chat configuration (chat name only, without sections validation)
     * @returns {Promise<Object>} Configuration object with chat info
     * @throws {Error} If config file is missing or invalid
     */
    async loadChatConfig() {
        // If full config already loaded, use it
        if (this.telegramConfig) {
            return this.telegramConfig;
        }
        ...
        try {
            ...
            this._validateChatConfig(downloadConfig);

            return downloadConfig;
        } catch (err) {
            throw new Error(`Failed to load chat config: ${err.message}`);
        }
    }
     ...
     */
    async getChatName() {
        const config = await this.loadChatConfig();
    ...
    async getDownloadConfig() {
        try {
            ...
            const downloadConfigFile = await fs.readFile(downloadConfigPath, 'utf8');
            const downloadConfig = yaml.load(downloadConfigFile);
            return { ...DEFAULT_DOWNLOAD_CONFIG, ...downloadConfig.settings };
        } catch (err) {
            if (err.code === 'ENOENT') {
                return { ...DEFAULT_DOWNLOAD_CONFIG };
            }
            throw new Error(`Failed to load download config: ${err.message}`);
        }
    }
}
