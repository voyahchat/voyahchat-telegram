...
class TelegramLogger {
    ...
    actionComplete(result, messageId, title) {
        if (this.isTest) return;
        const timestamp = this._getTimestamp();
        const tag = result.toUpperCase();
        // Final results get extra newline to separate topics visually
        const finalResults = ['UPDATED', 'CREATED', 'FAILED', 'UNCHANGED', 'RECREATED'];
        const suffix = finalResults.includes(tag) ? '\n' : '';
        console.log(`${timestamp} [${tag}] Message ${messageId}: ${title}${suffix}`);
    }

    /**
     * Display an in-place countdown on a single line and wait until it finishes.
     * On a TTY the remaining seconds are redrawn each second via carriage return;
     * when output is not a TTY (e.g. piped to a file) a single static line is
     * printed instead to avoid carriage-return noise.
     * @param {number|null} messageId - Message ID, or falsy to omit the "Message <id>:" prefix
     * @param {number} seconds - Countdown duration in seconds
     * @param {string} message - Message text shown before the seconds suffix
     * @returns {Promise<void>} Resolves when the countdown completes
     */
    countdown(messageId, seconds, message) {
        if (this.isTest) return Promise.resolve();

        const timestamp = this._getTimestamp();
        const prefix = messageId ? `Message ${messageId}: ` : '';
        const render = (remaining) => `${timestamp} [WAIT] ${prefix}${message} ${remaining}s...`;
        ...
        return new Promise((resolve) => {
            ...
            process.stdout.write(render(remaining));
