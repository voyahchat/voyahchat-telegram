...
class BotApi {
    ...
    /**
     * Check if error is "there is no text in the message to edit"
     * Occurs when editMessageText is called on a media (photo) message
     * @param {Error} err - Error to check
     * @returns {boolean} True if the target message has no text content
     */
    static isMessageHasNoText(err) {
        return !!(err && err.description &&
