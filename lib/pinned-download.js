...
/**
 * Remove first line (title) from markdown content
 * @param {string} content - Markdown content
 * @returns {string} Content without first line
 */
...
class PinnedDownload {
    ...
    async downloadMessage(chat, messageId) {
        try {
            ...
            if (err.message.includes('flood') || err.message.includes('FLOOD_WAIT')) {
                const waitTime = extractFloodWaitTime(err.message);
                await this.logger.countdown(messageId, waitTime, 'Rate limited. Waiting');
    ...
    async downloadTopic(topic) {
        ...
        try {
            ...
            // Convert to markdown
            let markdown = MarkdownConverter.fromTelegram(message.text, message.entities);

            // Remove title line if present
            markdown = stripTitleLine(markdown);
