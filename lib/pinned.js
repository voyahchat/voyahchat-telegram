...
const { LoggerGuard, pinnedActionFilter } = require('./logger-guard');
...
class PinnedSync {
    ...
    constructor(options = {}) {
        ...
        // Initialize logger guard
        this.loggerGuard = new LoggerGuard({
            enabled: !this.verbose,
            filter: pinnedActionFilter,
        });
    ...
    async init() {
        ...
        // Use LoggerGuard to suppress console output in non-verbose mode
    ...
    async getChat() {
        ...
        try {
            ...
            if (err.message.includes('flood') || err.message.includes('FLOOD_WAIT')) {
                ...
                await this.logger.countdown(null, waitTime, 'Rate limited. Waiting');
            ...
            if (topic.pinnedId) {
                ...
                try {
                    ...
                    if (messages.length > 0 && messages[0]) {
                        ...
                        if (this.compareContent(contentWithTitle, message) && !imageChanged) {
                            ...
                            await this.editMessage(chat, topic.pinnedId, contentWithTitle, imagePath, topic.topicId);
                            result.action = 'updated';
                            this.logger.actionComplete(result.action, topic.pinnedId, topic.title || topic.slug);
                        }
                    } else {
                        this.logger.debug(`Message ${topic.pinnedId} not found, creating new message`);
                        const newMessage = await this.publishMessage(chat, contentWithTitle, imagePath, topic.topicId);
    ...
    async close() {
        if (this.client) {
            ...
            // Use LoggerGuard to suppress console output in non-verbose mode
            await this.loggerGuard.run(async () => {
                ...
                try {
                    ...
                    if (!err.message.includes('TIMEOUT')) {
                        // Use withConsole to temporarily restore console for error logging
                        this.loggerGuard.withConsole(() => {
