...
class MarkdownConverter {
    ...
    static fromTelegram(text, entities = []) {
        ...
        for (const entity of sortedEntities) {
            ...
            // Normalize entity type from Telegram API className
            const type = this.normalizeEntityType(entity);
            ...
            case 'url':
                // Plain URLs - keep as is, they are auto-linked
                break;
            case 'strikethrough':
                markdown = markdown.substring(0, offset) +
                    `~~${entityText}~~` +
            ...
            case 'spoiler':
                markdown = markdown.substring(0, offset) +
                    ...
                    markdown.substring(offset + length);
                break;
            // Other entity types (mention, hashtag, etc.) - keep as plain text
            }
        }
    ...
    /**
     * Normalize Telegram entity type from className or type field
     * @param {Object} entity - Telegram entity object
     * @returns {string} Normalized type name
     */
    static normalizeEntityType(entity) {
        // If entity has className (from gramjs), use it
        if (entity.className) {
            const typeMap = {
        ...
        // If entity has type field (from Bot API format), use it directly
        return entity.type || '';
    }
     ...
     */
    static toTelegramHtml(markdown) {
        if (!markdown) {
            return '';
        }

        // Process links first (they may contain nested bold)
        // Replace [text](url) with <a href="url">processed_text</a>
        // where processed_text has bold converted to <b>
        let html = markdown.replace(/\[([^\]]+)\]\(([^)]+)\)/g, (match, linkText, url) => {
            // Process bold within link text
            const processedText = linkText.replace(/\*\*([^*]+)\*\*/g, '<b>$1</b>');
            return `<a href="${url}">${this._escapeHtml(processedText)}</a>`;
        });

        // Process remaining bold (outside links)
        html = html.replace(/\*\*([^*]+)\*\*/g, '<b>$1</b>');

        // Escape HTML special characters in plain text (but not in HTML tags we just created)
        // We need to escape only the parts that are not already HTML tags
        // Strategy: split by HTML tags, escape the text parts, reassemble
        ...
        html = parts.map(part => {
            ...
            // Otherwise, escape HTML special chars
