...
class BotSync {
    ...
    async syncTopic(topic) {
        ...
        try {
            ...
            // Include image content in hash if present
            let fullHash;
            if (imagePath) {
                ...
                fullHash = contentHash(localContent + '::image::' + imageBuffer.toString('base64'));
            } else {
                fullHash = localHash;
            }
            ...
            // Update config with new hash and botPinnedId (only if something changed)
            if (!this.dryRun && result.action !== 'unchanged') {
                await this.topicsConfig.updateBotPinnedId(topic.slug, result.messageId, fullHash);
            }
...
module.exports = { BotSync, contentHash };
